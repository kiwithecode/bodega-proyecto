...
| `07_tests_correcciones_pgtap.sql` en el SQL Editor | 35 pruebas de `fn_editar_lote`, `fn_quitar_jaba` y `fn_anular_lote`: el código se rearma con la fecha, no pisa lotes existentes, quitar una jaba recalcula el lote y respeta lo ya procesado, no anula lotes ya procesados, borra jabas y recepción vacía, deja constancia | Supabase con 01–05 y `07_correcciones.sql` |
